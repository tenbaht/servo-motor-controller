;----------------------------------------------------------------------------;
; AVR SERVOMOTOR CONTROLLER R0.2  (with position display)     (C)ChaN, 2004  ;
;----------------------------------------------------------------------------;
;
...
.def	_0	= r15	;ゼロレジスタ/Permanent zero register

.def	_PvEnc	= r14	;前回ポート
.def	_PvDir	= r13	;前回方向
.def	_PosH	= r12	;現在位置/Current position
.def	_PosM	= r11	;
.def	_PosL	= r10	;/
...
background:
	...
	in	_PvEnc, PIND		;Sample A:B quadrature signal into _PvEnc[1:0]
	swap	_PvEnc			;/
...
tap_speed:
	...
	sts	PvInt+0, r0	; To prevent reset wind up.
	sts	PvInt+1, r1	;
	cbr	_Flags, bit6	;/
...
tap_voltage:
	...
	adc	r18,_0
	ldi	ZL,14
	ldi	ZH,0		;digit counter
	clr	r19		;---- decimal string generating loop
	ldi	r20,24		;var1 /= 10;
	lsl	r16		;
	rol	r17		;
	rol	r18		;
	...
	inc	ZH
	cp	_0,r16		;if(var1 =! 0)
	cpc	_0,r17		; continue digit loop;
	cpc	_0,r18		;
	brne	PC-18		;/
	mov	r19,ZL
	cpse	ZH,r21
	...
	mov	ZL,r19
	clr	ZH
	subi	ZL, low(-t_7seg*2)
	sbci	ZH, high(-t_7seg*2)
	out	SREG, r0
	lpm
	ldi	ZL, 8
	sbi	PORTB,6
...
t_7seg:		.db	0x3f,0x06,0x5b,0x4f,0x66,0x6d,0x7d,0x07,0x7f,0x6f,0x38,0x76,0x73,0x79,0x40,0x00
